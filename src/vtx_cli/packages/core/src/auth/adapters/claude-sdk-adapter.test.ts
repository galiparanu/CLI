...
describe('ClaudeSDKAdapter', () => {
  ...
  beforeEach(() => {
      ...
      stdout: {
        on: vi.fn((event, callback) => {
          if (event === 'data') {
            ...
            setTimeout(() => callback(Buffer.from('{"content":"test response","stop_reason":"end_turn"}')), 10);
          }
        }),
      },
      stderr: {
    ...
    adapter = new ClaudeSDKAdapter('python3');
  });

  describe('getAuthMethod', () => {
    it('should return CLAUDE_SDK', () => {
  ...
  describe('validateDependencies', () => {
    it('should return true when Python is available', async () => {
      ...
      mockSpawn.mockImplementation((cmd, args) => {
        ...
        if (args && args[0] === '-c' && args[1]?.includes('import anthropic')) {
          const proc = {
            on: vi.fn((event, callback) => {
              if (event === 'close') {
                setTimeout(() => callback(0), 10);
    ...
    it('should return false when anthropic package is not installed', async () => {
      mockSpawn.mockImplementation((cmd, args) => {
        if (args && args[0] === '--version') {
          const proc = {
            on: vi.fn((event, callback) => {
  ...
  describe('authenticate', () => {
    it('should return success when dependencies are valid', async () => {
      ...
      expect(result.method).toBe(AuthMethodType.CLAUDE_SDK);
    });
  ...
  describe('sendRequest', () => {
    beforeEach(() => {
      // Mock successful dependency check
      mockSpawn.mockImplementation((cmd, args) => {
        if (args && (args[0] === '--version' || (args[0] === '-c' && args[1]?.includes('import')))) {
          const proc = {
            on: vi.fn((event, callback) => {
              if (event === 'close') {
                setTimeout(() => callback(0), 10);
